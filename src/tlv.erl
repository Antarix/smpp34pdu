...
pack_octstring_fixedlen(Tag, Val, Len) ->
  case tools:octstring_to_bin(Val, Len) of
    {error, _}=Err ->
      Err;
    Bin ->
...
pack_octstring_varlen(Tag, Val, {Min, Max}) ->
  case tools:octstring_to_bin(Val, {Min, Max}) of
    {error, _}=Err ->
      Err;
    Bin ->
      Len =
...
pack_octstring_nomax(Tag, Val) ->
  Len = byte_size(Val),
  Bin = tools:octstring_to_bin(Val, Len),
  <<Tag:?TLV_TAG_SIZE, Len:?TLV_LEN_SIZE, Bin/binary>>.
